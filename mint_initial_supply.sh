...
echo "Checking current APTC token supply..."
# Verify that the canister exists and is accessible
APTC_TOKEN_ID=$(dfx canister id APTC-token || echo "")
if [ -z "$APTC_TOKEN_ID" ]; then
    echo "❌ Error: Cannot find the APTC-token canister ID."
    echo "Please deploy the token first with './deploy_aptc_token.sh'"
    exit 1
fi
