...
import { agentRegistry, ICPService } from "../lib/icp-service";
import { Agent, TokenCard, Badge } from "../lib/types";

// Helper function to convert agent data to token card format
const agentToTokenCard = (agent: Agent): TokenCard => {
  // Create appropriate badges based on agent metrics
  const badges: Badge[] = [];

  // Add trending badge if agent has high usage
  if (agent.metrics.totalUsers > 1000) {
    badges.push({
      type: "trending" as const,
      value: `${Math.floor(agent.metrics.totalUsers / 100)}`,
  ...
  if (daysSinceCreation < 14) {
    ...
    const timeLabel = daysSinceCreation < 1 ? "1d" : daysSinceCreation + "d";
    badges.push({
      type: "new" as const,
      value: timeLabel,
  ...
  return {
    name: agent.name,
    symbol: `$${agent.token.substring(0, 6)}`,
    marketCap: `$${(agent.metrics.totalRevenue * 1000).toLocaleString()}`,
    change: `+${agent.metrics.stakingApr.toFixed(1)}%`,
    changeColor:
      agent.metrics.stakingApr > 15
        ? "rgba(83,227,122,1)"
        : "rgba(239,102,102,1)",
    backgroundColor: `bg-[${getAgentColor(agent.category)}]`,
    logoUrl: agent.imageUrl,
    socialCount: `${agent.metrics.totalUsers}`,
    badges: badges,
    hasLogo: true,
  };
};
...
const Index: React.FC = () => {
  const [topEarningAgents, setTopEarningAgents] = useState<Agent[]>([]);
  ...
  useEffect(() => {
    const fetchAgentData = async () => {
      try {
        setIsLoading(true);
        // Fetch agent data from our mock ICP service
        const topEarning = await ICPService.getTopEarningAgents();
        const trending = await ICPService.getTrendingAgents();
        const recent = await ICPService.getRecentAgents();

        setTopEarningAgents(topEarning);
  ...
  // Convert our mock agents to token cards
            ...
            <div className="w-[67%] max-md:w-full max-md:ml-0">
              <div className="grow max-md:max-w-full max-md:mt-10">
                {isLoading ? (
                  ...
                  <>
                    {/* Genesis Launches section shown in the screenshot */}
                    <GenesisList />
