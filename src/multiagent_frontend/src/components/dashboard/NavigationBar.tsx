  ...
  useEffect(() => {
    const searchAgents = async () => {
      ...
      try {
        ...
        try {
          sourceAgents = await agentRegistry.getAllAgents();
        } catch (error) {
          console.log("Failed to get agents from registry, using fallback data");
          sourceAgents = [];
        }
        ...
        if (filteredAgents.length > 0) {
          ...
          // If no results from direct filtering, use fallback data
